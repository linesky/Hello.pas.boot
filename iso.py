...
class BareboneBuilder:
    ...
    def build_kernel(self):
        ...
        self.execute_command("nasm -f elf32 -o /tmp/codes.o codes.s",False)
        fff=f'cp -f "$1" /tmp/kernel.pas'.replace("$1",filename)
        
        self.execute_command(fff,True)
        fff=f'fpc -Cn -CcCDECL -O2 -Xs -Xs -Xt -Pi386 -Tlinux  "/tmp/kernel.pas"'
        
        self.execute_command(fff,True)
        self.execute_command("ld -T link.ld /tmp/codes.o /tmp/kernel.o -o /tmp/hello.com -nostdlib",True)
